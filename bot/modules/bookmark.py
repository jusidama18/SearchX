...
from bot import app, BOOKMARKS, DATABASE_URL
...
def addbookmark(update, context):
    ...
    if len(args) > 2:
        ...
        sendMessage(
            "<b>Send a Drive Key and a Drive ID along with command</b>",
            context.bot,
            update.message,
        )
        return
...
def rembookmark(update, context):
    ...
    if len(args) > 1:
        ...
        sendMessage(
            "<b>Send a Drive Key along with command</b>", context.bot, update.message
        )
        return
    if drive_key in BOOKMARKS:
...
addbm_handler = CommandHandler(
    BotCommands.AddBookmarkCommand, addbookmark, filters=CustomFilters.owner_filter
)
rembm_handler = CommandHandler(
    BotCommands.RemBookmarkCommand, rembookmark, filters=CustomFilters.owner_filter
)
bookmarks_handler = CommandHandler(
    BotCommands.BookmarksCommand,
    bookmarks,
    filters=CustomFilters.authorized_user | CustomFilters.authorized_chat,
)
app.add_handler(addbm_handler)
app.add_handler(rembm_handler)
app.add_handler(bookmarks_handler)
